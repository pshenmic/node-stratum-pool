...
var pool = module.exports = function pool(options, authorizeFn, logger) {
        ...
        var generateProgress = function () {
            ...
            _this.daemon.cmd('getblockchaininfo', [], function (results) {
                ...
                _this.daemon.cmd('getpeerinfo', [], function (results) {
                    ...
                    if (totalBlocks !== 0) {
                        percent = (blockCount / totalBlocks * 100).toFixed(2);
                    }

                    logger.warn('Downloaded %s% of blockchain from %s peers', percent, peers.length);
                });
        ...
        for (var r in options.rewardRecipients) {
            ...
            try {
                if (r.length === 40){
                    ...
                    let addressType = util.detectAddressType(r, options.coin);
                    rObj.script = util.addressToScript(r, addressType);
                }
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            ...
            options.poolAddressScript = (function () {
                switch (options.coin.reward) {
                    ...
                    case 'POW':
                        let address = rpcResults.validateaddress.address;
                        let addressType = util.detectAddressType(address, options.coin);
                        return util.addressToScript(address, addressType);
                }
            })();

            options.testnet = rpcResults.getblockchaininfo.testnet;
            options.protocolVersion = rpcResults.getblockchaininfo.protocolversion;

            options.initStats = {
                connections: rpcResults.getblockchaininfo.connections,
    ...
    function GetBlockTemplate(callback) {
        _this.daemon.cmd('getblocktemplate',
            ...
            function (result) {
                if (result.error) {
                    logger.debug("result.error = %s", JSON.stringify(result));
                    //todo to string interpolation, i'm tired
