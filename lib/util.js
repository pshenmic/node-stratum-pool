...
exports.addressToP2SHScript = function (addr) {
    const decoded = decodeAddress(addr);

    // 0xa9       0x14                               0x87
    // OP_HASH160 OP_DATA_20 <HASH160_REDEEM_SCRIPT> OP_EQUAL
    const SCRIPT_HASH = decoded.slice(1, -4);
    return Buffer.concat([Buffer.from([0xa9, 0x14]), SCRIPT_HASH, Buffer.from([0x87])]);



};

exports.addressToP2PKHScript = function (addr) {

    const decoded = decodeAddress(addr);

    // 0x76   0xa9       0x14       0x[XXXXXXXX] 0x88           0xac
    // OP_DUP OP_HASH160 OP_DATA_20 <pubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
 ...
 */
exports.addressToScript = function (address, addressType) {
    if (!address || !addressType) {
        throw new Error("Address or type is undefined");
    } else {
        switch (addressType) {
            case "P2SH" :
                return exports.addressToP2SHScript(address);
            case "P2PKH":
                return exports.addressToP2PKHScript(address);
            default:
                throw new Error("Address is type is not known");
        }
    }
};
...
/**
 * Returns P2SH or P2PKH
 * @param address {string}
 * @param coinConfig {object}
 */
function detectAddressType(address, coinConfig) {
    if (!address || !coinConfig || !coinConfig.versionByte) {
        throw new Error("Address or coinConfig is null");
    } else {
        console.log(address);
        let addressVersionByte = exports.getVersionByte(address);

        let P2SHVersionByte = Buffer.alloc(1);
        P2SHVersionByte.writeUInt8(coinConfig.versionByte.P2SH);

        let P2PKHVersionByte = Buffer.alloc(1);
        P2PKHVersionByte.writeUInt8(coinConfig.versionByte.P2PKH);

        console.log(addressVersionByte, P2SHVersionByte, P2PKHVersionByte);

        if(addressVersionByte.equals(P2SHVersionByte)) {
            return "P2SH";
        } else if(addressVersionByte.equals(P2SHVersionByte)) {
            return "P2PKH";
        } else {
            throw new Error("Address type could not be recognized of address: " + address);
        }
    }
